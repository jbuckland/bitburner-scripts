...
export class SoundPlayer {
    ...
    constructor(filterNode?: AudioNode) {
    ...
    play(freq: number, vol: number, wave?: OscillatorType, when?: number) {
    ...
    setFrequency(val: number, when?: number) {
        if (this.oscillator) {
            if (when) {
    ...
    setVolume(val: number, when?: number) {
        if (when) {
