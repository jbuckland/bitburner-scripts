...
import { DebugLevel, HacknetMode, TOAST_DURATION, TOAST_VARIANT } from 'utils/consts';
import { debugLog, formatBigNumber, formatCurrency, getSettings, round, timestamp } from 'utils/utils';
...
const COST_THRESHOLD_TO_NOTIFY = 1e9;
...
export async function main(ns: NS) {
    ...
    while (true) {
        ...
        if (shouldBuyNode && availableMoney > newNodeCost) {
            ...
            if (settings.hacknetMode === HacknetMode.hacking) {
    ...
    function buyUpgrade(upgrade: HacknetUpgrade): boolean {
        ...
        if (msg.length > 0) {
            ...
            if (upgrade.upgradeCost > COST_THRESHOLD_TO_NOTIFY) {

                ns.toast(`${msg} for ${formatCurrency(upgrade.upgradeCost)}`, TOAST_VARIANT.success, TOAST_DURATION);
